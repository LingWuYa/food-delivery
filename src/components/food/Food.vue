...
export default {
  ...
  methods: {
    show () {
      ...
      this.$nextTick(() => {
        // 在商品详情页展开状态，且dom渲染完毕后计算food盒子下的food-content盒子高度是否超出父盒子，需不需要调用better-scroll滚动页面
        if (!this.scroll) {
    ...
    addFood (target) {
      this.$emit('add', target) // 将food中的cartcontroll组件递交上来的目标el经过这个函数传给父组件goods
    },
    needShow (type, text) {
  ...
  .food
    ...
    .food-content
      .image-header
        ...
        width: 100%
        height: 0
        padding-top: 100% // 这里是相对盒子宽度的100%，从而达到盒子/图片等宽高并在加载完成前就占位
        img
          position: absolute
          top: 0
          left: 0
